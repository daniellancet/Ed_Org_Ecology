...
df_Main_Category = df.groupby("Main Category").agg({'count': 'sum', 'Broader Code': 'first', 'Broadest Code': 'first'}).reset_index()
df_Broader_Code = df.groupby("Broader Code").agg({'count': 'sum', 'Main Category': 'first', 'Broadest Code': 'first'}).reset_index()
df_Broadest_Code = df.groupby("Broadest Code").agg({'count': 'sum', 'Main Category': 'first', 'Broader Code': 'first'}).reset_index()

scholarship_index = df_Main_Category[df_Main_Category['Main Category'] == 'Scholarship'].index[0]
fundraising_index = df_Main_Category[df_Main_Category['Main Category'] == 'Fundraising'].index[0]
...
labels = list(df_Main_Category['Main Category'])
labels.extend(df_Broader_Code['Broader Code'])
labels.extend(df_Broadest_Code['Broadest Code']) 

source = list(range(0, len(df_Main_Category['Main Category']) + len(df_Broader_Code['Broader Code'])))

target_names = list(df_Main_Category['Main Category'])
...
def find_index_main_categories(row): 
    for i in range(len(df_Main_Category['Main Category']), len(df_Main_Category['Main Category']) + len(df_Broader_Code['Broader Code'])):
...
def find_index_broader_categories(row): 
    for i in range(len(df_Main_Category['Main Category']) + len(df_Broader_Code['Broader Code']), len(target_names)):
...
fig.update_layout(
    title_text="Simple Sankey Diagram", 
    font_size=10,
    width=800,  
    height=800  
)
