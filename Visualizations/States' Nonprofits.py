...
df = df.groupby(['f990_org_addr_state']).size().reset_index(name = "Educational Nonprofits") ## group by city and state, counting instances of "CA" and "Los Angeles" appearing together for example

df = df.sort_values(by='Educational Nonprofits', ascending=False) ## sort locations by number of educational nonprofits within it

df['State'] = df['f990_org_addr_state'] ## combine city and state names to make labels of graph, for example "Berkeley, CA"
...
sns.barplot(x='Educational Nonprofits', y='State', data=df, orient='h', palette=solid_muted_palette) ## create barplot, orient horizontally, use palette created earlier
